...
//**** Accessible par toutes les autres classe, c'est le moteur de l'appli
//**** et elle gere la partie graphique 
...
public class Scene extends JPanel{
	...
	private int hauteurPlafond; // Hauteur courant du plafond
	...
	// Tableau : permet d'enregistrer les objets du jeu
...
//		tortue = new Tortue(950, 243);
		...
		// Relie le clavier à la scene
		this.setFocusable(true);
		this.requestFocusInWindow();
		this.addKeyListener(new Clavier()); // ecouteur du clavier
		
		score = new Score();
		police = new Font("Arial", Font.PLAIN, 13);
		compteARebours = new CompteARebours();
		
		// Relie le chrono à la scene
	...
	public void deplacementFond() {
		if(this.xPos >= 0 && this.xPos <= 4430) {
			...
			//Remise à zero des abscisses pour rotation du fond
...
//	}
	...
	public void paintComponent(Graphics g) {
		//dessin de toutes les images visibles à l'ecran
		
		super.paintComponent(g);
		Graphics g2 = (Graphics2D)g;
		
		// Detection contact Mario + pieces
		for(int i = 0; i < this.tabPieces.size(); i++) {
			...
			if(this.mario.proche(this.tabPieces.get(i))) {
				if(this.mario.contactPiece(this.tabPieces.get(i))) {
					...
					this.score.setNbrPieces(this.score.getNbrPieces() + 1);
				}
			}
		}		
		
		// Detection des contacts (champ) personnages entre eux 
		...
		// Detection des contacts (tortue) personnages entre eux 
		...
		// Detection contact mario perssonnage		
		// Champignon
		for(int i = 0; i < this.tabChamps.size(); i++) {
			if (this.mario.proche(this.tabChamps.get(i)) 
				&& this.tabChamps.get(i).isVivant() == true) {
		...
		//Tortues
		for(int i = 0; i < this.tabTortues.size(); i++) {
			if (this.mario.proche(this.tabTortues.get(i)) 
				&& this.tabTortues.get(i).isVivant() == true) {
		...
		// Detection des contacts avec un objet
		for(int i = 0; i < this.tabObjets.size(); i++) {
			...
			// champigon
			for(int j = 0; j < this.tabChamps.size(); j++) {
				if(this.tabChamps.get(j).proche(this.tabObjets.get(i))) { //////////******ERREUR
		...
		g2.setFont(police);
		g2.drawString(this.score.getNbrPieces() + " pièce(s) trouvée(s) sur " + this.score.getNBR_TOTAL_PIECE(), 460, 25);
